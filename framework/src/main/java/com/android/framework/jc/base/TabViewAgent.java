...
public class TabViewAgent implements TabLayout.BaseOnTabSelectedListener {
    ...
    private final int[] mSelectedImgList;
    private final int[] mUnselectedImgList;
    private final int[] mTitleList;

    public TabViewAgent(Context context, TabLayout tabLayout, int[] selectImgList, int[] unselectedImgList) {
        this(context, tabLayout, selectImgList, unselectedImgList, null);
    }

    public TabViewAgent(Context context, TabLayout tabLayout, int[] selectImgList, int[] unselectedImgList, int[] titleList) {
    ...
    public View getCustomView(int position, CharSequence title) {
        return new DefaultView(mContext, title, mSelectedImgList[position], mUnselectedImgList[position]);
    }
    ...
    public void prepare(PagerAdapter adapter) {
        ...
        for (int i = 0; i < mTabLayout.getTabCount(); i++) {
            ...
            CharSequence title;
            if (initTitle) {
                title = ResourcesUtils.getString(mTitleList[i]);
            } else {
...
//                float cVal = 0.8f;
                view.setAlpha(0.9f + (cVal - 1f) * (0.5f / 0.1f));
                view.setScaleX(cVal);
                view.setScaleY(cVal);
            });
        }
